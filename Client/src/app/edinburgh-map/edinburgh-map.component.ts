...
@Component({
  ...
  templateUrl: './edinburgh-map.component.html',
  styleUrls: ['./edinburgh-map.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class EdinburghMapComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this._edinburghDataManager.getDistricts()
    ...
    this._edinburghDataManager.getDistrict().subscribe((district: District) => this.ward = district);

    this._edinburghDataManager.getLatestTweet().subscribe((tweet: Tweet) => {
      if (tweet !== undefined) {
        if (tweet.id === ('edinburgh-boundary')) {
    ...
    this._edinburghDataManager.getMapTopology().subscribe((topology: FeatureCollection<any>) => this.drawMap(topology));
  }
  ...
  private initVariables(): void {
    ...
    this.svg = d3.select('#edinburgh-map')
      .append('svg')
      .attr('id', 'mapp')
    ...
    this.offsetL = document.getElementById('edinburgh-map').offsetLeft + 20;
    this.offsetT = document.getElementById('edinburgh-map').offsetTop + 20;

    this.path = d3.geo.path().projection(this.projection);

    this.tooltip = d3.select('#edinburgh-map')
      .append('div')
  ...
  public drawMap(topology: FeatureCollection<any>): void {
    console.log('Edinburgh map drawing', topology);
  ...
  private drawGlasgowOutline(t: FeatureCollection<any>): void {
    ...
    // Only returns the arcs that aren't shared by wards i.e the outer bounds
      ...
      .attr('d', this.path)
      .attr('stroke', () => this.colour(this.wards['edinburgh-boundary'].average))
      .attr('id', 'edinburgh-boundary')
      .attr('class', 'selected')
      .on('click', this.setData)
      .on('mousemove', this.showTooltip)
      .on('mouseout', () => {
  ...
  private setData = (e: Feature<any> | MultiLineString): void => {
    const id = this.isFeature(e) ? e.properties.WD13CD : 'edinburgh-boundary';
    this._edinburghDataManager.setDistrict(id);
  }
  ...
  private showTooltip = (d: Feature<any> | MultiLineString): void => {
    const label = (this.isFeature(d) ? d.properties.WD13NM : 'Edinburgh') +
      '<br> ' + this.wards[this.isFeature(d) ? d.properties.WD13CD : 'edinburgh-boundary'].prettyAverage + '% Happy';
